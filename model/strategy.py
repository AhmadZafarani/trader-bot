...
class ADX12(Strategy):
    def strategy_works(self) -> bool:
        if opening_con1_num_of_candles == 3:
            if (
                    self.candles[self.moment.candle_id-2].open_price > self.candles[self.moment.candle_id-2].moving12 and
                    ...
                    ((self.candles[self.moment.candle_id-4].close_price - self.candles[self.moment.candle_id-4].moving12) / (
                        self.candles[self.moment.candle_id-4].close_price - self.candles[self.moment.candle_id-4].open_price)) * 100 > opening_con1_min_first
            ):
                if(opening_con2_di_method.__eq__("positive")):
                    if(
                        self.candles[self.moment.candle_id-2].DI_plus > self.candles[self.moment.candle_id-2].DI_minus and
                        self.candles[self.moment.candle_id -
                                     2].adx > opening_con2_min_adx
                    ):
                        return True
                elif(opening_con2_di_method.__eq__("increasing")):
                    if(
                            self.candles[self.moment.candle_id-2].DI_plus - self.candles[self.moment.candle_id-2].DI_minus > self.candles[self.moment.candle_id-3].DI_plus - self.candles[self.moment.candle_id-3].DI_minus and
                            self.candles[self.moment.candle_id -
                                         2].adx > opening_con2_min_adx
                    ):
                        return True
            ...
            if (
                    ...
                    ((self.candles[self.moment.candle_id-3].close_price - self.candles[self.moment.candle_id-3].moving12) / (
                        self.candles[self.moment.candle_id-3].close_price - self.candles[self.moment.candle_id-3].open_price)) * 100 > opening_con1_min_first
            ):
                if(opening_con2_di_method.__eq__("positive")):
                    if(
                        self.candles[self.moment.candle_id-2].DI_plus > self.candles[self.moment.candle_id-2].DI_minus and
            ...
            if (
                    ...
                    ((self.candles[self.moment.candle_id-2].close_price - self.candles[self.moment.candle_id-2].moving12) / (
                        self.candles[self.moment.candle_id-2].close_price - self.candles[self.moment.candle_id-2].open_price)) * 100 > opening_con1_min_first
            ):
                if(opening_con2_di_method.__eq__("positive")):
                    if(
                        self.candles[self.moment.candle_id-2].DI_plus > self.candles[self.moment.candle_id-2].DI_minus and
    ...
    def start_strategy(self):
        ...
        self.ADX = [self.candles[self.moment.candle_id-2].adx, self.candles[self.moment.candle_id -
                                                                            2].DI_plus, self.candles[self.moment.candle_id-2].DI_minus]
        self.C = self.candles[self.moment.candle_id - 1]
        self.Mom_str = self.moment.__str__()
        # self.MA = [self.candles[ self.candles[self.moment.candle_id - 1].moving12 , self.candles[self.moment.candle_id - 2].moving12 , self.candles[self.moment.candle_id - 3].moving12 , self.moment.candle_id - 5].moving12 ]
        ...
        elif(lock_method.__eq__("lock_to_fin")):
            lock_strategies["adx"] = [ADX12, 0]

    def sell_method1(self, candle_id: int) -> bool:
        if closing_meth1_num_of_candles == 3:
            if (
                    self.candles[candle_id-2].open_price < self.candles[candle_id-2].moving12 and
                    ...
                    ((self.candles[candle_id-4].moving12 - self.candles[candle_id-4].close_price) / (
                        self.candles[candle_id-4].open_price - self.candles[candle_id-4].close_price)) * 100 > closing_met1_min_first
            ):
                return True
            ...
            if(
                    self.candles[candle_id-2].close_price < self.candles[candle_id-2].open_price and
    ...
    def sell_method3(self) -> bool:
        if(((self.moment.price - self.buy_price) / self.buy_price) * 100 > profit_limit or ((self.moment.price - self.buy_price) / self.buy_price) * 100 < loss_limit):
            return True
        return False
    ...
    def fin_and_before(self):
        ...
        ID : {self.id}
        """)
        if(lock_method.__eq__("lock_to_fin")):

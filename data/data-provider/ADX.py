...
with open('data/adx.csv', 'w', newline='') as file:
    ...
    for i in range(len(data)):
        if i < 2:
            ...
            if i < 16:
                writer.writerow([0.0, round(DIp[i - 2] , 2), round(DIn[i - 2] , 2)])
            else:
                writer.writerow([round(ADX[i - 15] , 2), round(DIp[i - 2] , 2), round(DIn[i - 2] , 2)])
